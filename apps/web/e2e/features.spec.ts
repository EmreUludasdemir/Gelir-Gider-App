...
test.describe('PDF Upload Review', () => {
  ...
  test('shows import review after a successful pdf upload', async ({ page }) => {
    ...
    await expect(page.getByText('Incele').first()).toBeVisible()
    await expect(page.getByText('Parser guveni inceleme gerektiriyor')).toBeVisible()
    await expect(page.getByText('Akilli kategori hamleleri')).toBeVisible()
    await page.getByRole('button', { name: /nisan-ekstre\.pdf/i }).click()
    await expect(page.getByText('Bu dosya parser tarafinda hata verdi.')).toBeVisible()
    await page.getByRole('button', { name: /mart-ekstre\.pdf/i }).click()
