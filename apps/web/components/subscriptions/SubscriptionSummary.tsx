...
export function SubscriptionSummary({ subscriptions: propSubs }: SubscriptionSummaryProps) {
    ...
    useEffect(() => {
        if (propSubs) {
            ...
            // Mock data
            const mockSubs = [
                { id: '1', name: 'Netflix', amount: 149.99, frequency: 'monthly', category: 'EÄŸlence', nextPayment: '2024-12-20' },
                { id: '2', name: 'Spotify', amount: 59.99, frequency: 'monthly', category: 'EÄŸlence', nextPayment: '2024-12-15' },
                { id: '3', name: 'iCloud', amount: 44.99, frequency: 'monthly', category: 'Teknoloji', nextPayment: '2024-12-22' },
                { id: '4', name: 'Spor Salonu', amount: 299, frequency: 'monthly', category: 'SaÄŸlÄ±k', nextPayment: '2024-12-25' },
            ];
    ...
    const getCategoryColor = (category: string) => {
        switch (category) {
            case 'EÄŸlence': return 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400';
            case 'Teknoloji': return 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400';
            case 'SaÄŸlÄ±k': return 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400';
        ...
        <div className="bg-card rounded-2xl p-6 shadow-sm border border-border">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="text-right">
                    <p className="text-sm text-muted-foreground">AylÄ±k</p>
                    <p className="text-xl font-bold text-red-500 dark:text-red-400">
                        â‚º{totalMonthly.toFixed(2)}
                    </p>
                </div>
            </div>
            ...
            <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg flex items-start gap-2">
                ...
                <p className="text-sm text-amber-700 dark:text-amber-300">
                    YÄ±llÄ±k: â‚º{(totalMonthly * 12).toFixed(0)} harcÄ±yorsunuz. GÃ¶zden geÃ§irin!
                </p>
            </div>
        </div>
    );
