...
export function StreakCounter({ currentStreak, longestStreak, compact = false }: StreakProps) {
    ...
    const getStreakMessage = () => {
        if (streak === 0) return 'Bugün ilk işlemini ekle! 🚀';
        if (streak < 7) return `${streak} günlük serin var! Devam et!`;
        if (streak < 30) return `${streak} gün! Sen bir yıldızsın! ⭐`;
        return `${streak} gün! İnanılmazsın! 🏆`;
    };
    ...
    if (compact) {
        return (
            <div className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-100 to-yellow-100 dark:from-orange-900/30 dark:to-yellow-900/30 rounded-full">
                ...
                <span className="text-sm text-orange-600 dark:text-orange-400">gün</span>
            </div>
        );
    }
            ...
            {streak > 0 && streak < 7 && (
                <div className="mt-4 p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                    <p className="text-sm text-orange-700 dark:text-orange-300">
                        {7 - streak} gün daha devam et ve "Düzenli" rozetini kazan! 🏅
                    </p>
                </div>
            )}
        </div>
